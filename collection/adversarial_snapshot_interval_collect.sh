...
SNAP_INTERVAL=(1 2 4 8 16 32 64)
NSAMPLES=5
...
for impl in ${IMPLS[@]}; do
    ...
    for ninserts in ${NINSERTS[@]}; do
        for snapinterval in ${SNAP_INTERVAL[@]}; do
            ...
            for ((s=0;s<NSAMPLES;s++)); do
                ...
                $scriptsdir/tools/run_bench.sh -c $config -o "$outdir/${impl}_${ro}_${nclients}c_${nworkers}w_${ninserts}i_${snapinterval}s_${sample}" -b $benchmark -r "$roimpl" -s "$snapinterval"
                sleep 5
            done
        done
    done
