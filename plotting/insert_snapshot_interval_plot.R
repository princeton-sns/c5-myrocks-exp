...
n_clients <- data %>%
    ...
    select(snap_interval, n_clients)

summary <- data %>%
    semi_join(n_clients) %>%
    filter(server != "Primary") %>%
    group_by(server, n_clients, snap_interval, n_workers) %>%
    summarize(
    ...
    group_by(server, n_clients, snap_interval) %>%
    mutate(
...
p <- bar_chart(summary,
  ...
  xtitle = "Snapshot Interval (txns)", ytitle = "Relative Commit Rate"
)
