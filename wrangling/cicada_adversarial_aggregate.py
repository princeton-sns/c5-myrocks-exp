...
def process_throughputs(inputdir):
    ...
    ninserts_re = re.compile(r".*__inserts_per_txn@(\S+?)__.*")
    ...
    for entry in os.scandir(inputdir):
        if entry.is_file() and not re.match(r".*\.failed-*", entry.path) and not re.match(r".*\.csv", entry.path):
            ...
            ninserts = ninserts_re.match(entry.path).group(1)
            ...
            newdir = "{}_{}_{}c_{}w_{}i_{:02.0f}".format(impl, ro_impl, nclients, nworkers, ninserts, int(i))
...
def main():
    ...
    os.system("./adversarial_aggregate.sh -i {} -o {}".format(inputdir, outdir))
