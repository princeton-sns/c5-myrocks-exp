...
def get_client_host(config):
    return config["host_format_str"] % ("client",
...
def ssh_args(command, remote_user, remote_host):
    return ssh_opts() + ['%s@%s' % (remote_user, remote_host), command]
...
def get_exp_net_interface(remote_user, remote_host):
    return run_remote_command_sync('cat /var/emulab/boot/ifmap | awk \'{ print $1 }\'', remote_user, remote_host).rstrip().splitlines()[-1]
